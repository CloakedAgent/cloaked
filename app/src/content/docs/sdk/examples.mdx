...
Enable and spend USDC with per-token constraints:
...
// Owner enables USDC with constraints
...
// Agent spends USDC (same as SOL but with mint)
...
// Check USDC balance
const usdcBalance = await agent.getTokenBalance(USDC_MINT);
console.log(`USDC: ${usdcBalance / 1e6}`);
```

<Callout type="info">
USDC uses 6 decimals, so 1 USDC = 1,000,000 native units. Token constraints are tracked separately from SOL.
</Callout>
